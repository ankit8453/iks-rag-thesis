...
_SYSTEM_INSTRUCTIONS: str = (
    ...
    "\n"
    "These treatises are GENERAL and SYMPTOM-BASED: they organise plant "
    "affliction by observable symptom (and its underlying cause), not by crop "
    "species, and a remedy given for a symptom applies across plants showing "
    "it. Your query must therefore be led by the SYMPTOM, not by the crop.\n"
    "\n"
    "RULES:\n"
    ...
    "we are looking for.\n"
    "3. The query MUST describe the visual symptoms of the EXACT disease "
    "named in the structured context below — never a generic illness. "
    "Translate that specific disease into the symptoms IT actually produces "
    "on the plant. Match the disease to its symptom family, for example: "
    "rust -> orange-to-brown powdery pustules; scab -> dark rough corky "
    "lesions; powdery mildew -> white powdery coating; blight -> spreading "
    "brown necrotic patches; leaf spot / Septoria -> distinct dark spots "
    "with pale or ringed centres; mold -> fuzzy discoloured patches; "
    "rot / black rot -> dark sunken decaying areas; mosaic or virus -> "
    "mottled yellow-green discoloration and curling; mite damage -> fine "
    "stippling and bronzing. Pick the description that fits the named "
    "disease and do NOT reuse a phrase for a disease it does not fit.\n"
    "4. Phrase the symptoms and the soil in descriptive, observational "
    "vocabulary the classical corpus would use (its appearance, fertility, "
    "and feel) rather than the modern technical labels themselves.\n"
    "5. LEAD WITH THE SYMPTOM. Open the query with the observed condition, "
    "and treat the crop as light background context at most — never as the "
    "thing being searched for. Do NOT write queries of the form \"what does "
    "<crop> disease look like\" or \"remedies for <crop>\"; the texts are not "
    "indexed by crop and such queries retrieve poorly. Naming the crop once, "
    "late in the sentence, is acceptable; making it the subject is not.\n"
    "6. Stay strictly within the structured context provided. Invent no "
    "extra symptoms, no extra soil properties, and no farmer narrative "
    "beyond what is supplied.\n"
    "7. If a suspected cause is supplied, work it into the query (e.g. with "
...
class LLMMediatedStrategy:
    ...
    def _build_prompt(self, context: MultimodalContext) -> str:
        ...
        return (
            ...
            "STRUCTURED CONTEXT:\n"
            f"- Crop (background context only — the texts are not crop-specific): {crop}\n"
