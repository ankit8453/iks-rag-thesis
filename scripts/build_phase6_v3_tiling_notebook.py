"""Generate ``notebooks/phase6_soil_training_v3_tiling.ipynb`` (Phase 6 V3-tiling Part 2).

V3-tiling runs the V2 training recipe **byte-for-byte** on a swapped
texture dataset (``iks-soil-texture-tiled`` instead of
``iks-soil-texture-irsid-vit``). The only thing different vs the V2
notebook is:

- Cell 1 documents the abandoned-3-stage V3 history and the V3-tiling
  ship criteria.
- Cell 6 points texture at the tiled dataset.
- Cell 9's training loop calls ``health_check`` after the first epoch.
- Cell 12 prints a V2 vs V3-tiling table + automatic SHIP/KEEP verdict
  and reports both **patch-level** and **image-level (majority vote)**
  texture accuracy so the reader can see how much of the gain — if any
  — is real per-image vs. just per-patch.

Generated artefact has exactly 13 cells.
"""
...
NOTEBOOK_PATH = PROJECT_ROOT / "notebooks" / "phase6_soil_training_v3_tiling.ipynb"

REPO_HTTPS_URL = "https://github.com/ankit8453/iks-rag-thesis.git"
REPO_LOCAL_PATH = "/content/iks-rag-thesis"

# V2 baseline (TTA test) from iks-soil-multitask-v2/eval_metrics_test.json
V2_BASELINE = {
...
# --------------------------------------------------------------------- #
# Cell 1 — Title + V2 baseline + history + criteria
# --------------------------------------------------------------------- #
md(
...
# --------------------------------------------------------------------- #
# Cell 5 — V3-tiling configuration markdown
# --------------------------------------------------------------------- #
md(
...
# --------------------------------------------------------------------- #
# Cell 6 — Load 3 HF datasets (texture is the tiled one)
# --------------------------------------------------------------------- #
code(
...
# --------------------------------------------------------------------- #
# Cell 7 — V2 transforms + per-task loaders (UNCHANGED from V2)
# --------------------------------------------------------------------- #
code(
...
# --------------------------------------------------------------------- #
# Cell 8 — Model + optimizer + scheduler + scaler + V3-tiling ckpt mgr + resume
# --------------------------------------------------------------------- #
code(
...
# --------------------------------------------------------------------- #
# Cell 9 — Training loop (V2-identical) + epoch-1 health check
# --------------------------------------------------------------------- #
code(
...
# --------------------------------------------------------------------- #
# Cell 10 — Markdown: TTA test header
# --------------------------------------------------------------------- #
md(
...
# --------------------------------------------------------------------- #
# Cell 11 — TTA test eval (patch-level)
# --------------------------------------------------------------------- #
code(
...
# --------------------------------------------------------------------- #
# Cell 12 — Image-level texture eval (majority vote) + V2 vs V3-tiling table
# --------------------------------------------------------------------- #
code(
...
# --------------------------------------------------------------------- #
# Cell 13 — Closing markdown + decision tree
# --------------------------------------------------------------------- #
md(
    "## V3-tiling complete\n"
    "\n"
